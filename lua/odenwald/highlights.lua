local c = require('odenwald.colors')
local cfg = vim.g.odenwald_config
local util = require('odenwald.util')
...
function M.setup()
    ...
    if color_name:sub(1, 1) == '$' then
      ...
      if not color_name then
        vim.schedule(function()
          vim.notify(
            'odenwald.nvim: unknown color "' .. name .. '"',
  ...
  for group_name, group_settings in pairs(vim.g.odenwald_config.highlights) do
